...
class CircularProgressDrawable(context: Context) :
    Drawable(), Animatable {
    /**
     * @hide
     */
     ...
     */
    var colorSchemeColors: IntArray
        get() = mRing.colors
        set(colors) {
            mRing.colors = colors
    ...
    private fun applyFinishTranslation(
        ...
        val targetRotation =
            (Math.floor(ring.startingRotation / MAX_PROGRESS_ARC.toDouble())
                    + 1f).toFloat()
     ...
     */
    init {
        mResources = Preconditions.checkNotNull(context)
            .resources
        mRing = Ring()
        mRing.colors = COLORS
